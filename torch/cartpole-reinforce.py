...
def replay(model, optimizer, replay_buf, gamma):
    G = []
    for t in range(len(replay_buf)):
        g = 0
        for k in range(t + 1, len(replay_buf)):
            g += (gamma ** (k - t - 1)) * replay_buf[t][0]
        G.append(g)
    G = torch.Tensor(G)
    G = G/G.max()

    action_probs = torch.stack([aprob for r, aprob in replay_buf])
    loss = - torch.sum(torch.log(action_probs) * G)
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
    return model, optimizer
...
def main():
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            total_reward += reward
            replay_buf.append((reward, action_prob[action]))
            state = next_state
        # Update model
        model, optimizer = replay(model, optimizer, replay_buf, gamma)
