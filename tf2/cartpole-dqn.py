...
def create_model(env):
    ...
    model.add(keras.layers.Dense(50, input_dim=env.observation_space.shape[0], activation=keras.layers.LeakyReLU()))
    model.add(keras.layers.Dense(100, activation=keras.layers.LeakyReLU()))
    model.add(keras.layers.Dense(env.action_space.n))
    model.compile(loss=keras.losses.MeanSquaredError(), optimizer=keras.optimizers.Adam(lr=0.001))
    return model

def replay(model, target_model, replay_buf, gamma):
    ...
    for obs, reward, done, action, nxt_obs in batch:
        ...
        if done:
            ...
            q_nxt = target_model.predict(nxt_obs.reshape(1, 4))[0]
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            if random.random() < epsilon:
                ...
                action = np.argmax(model.predict(observation.reshape(1, 4))[0])
            ...
            # Update model
            model = replay(model, target_model, replay_buf, gamma) 
